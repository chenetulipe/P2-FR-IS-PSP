...
def extract_cpk_from_iso(iso_path: str, out_dir: Path, log_fn, pspdecrypt_path: str = "") -> str:
    ...
    if out_eboot.exists():
        with open(out_eboot, "rb") as f:
            magic = f.read(4)
        
        if magic == b"\x7fELF":
            if log_fn: log_fn("  L'EBOOT est deja au format ELF decrypte ! Copie directe.", "ok")
            import shutil
            shutil.copy2(out_eboot, eboot_dec_out)
        else:
            import subprocess
            # Cherche le script decrypt_eboot.py
            repo_root = Path(__file__).resolve().parent.parent.parent.parent.parent
            tool_script = repo_root / "decrypt_eboot.py"
            
            if not tool_script.exists():
                tool_script = Path(__file__).resolve().parent.parent.parent / "decrypt_eboot.py"
                
            if not tool_script.exists():
                tool_script = repo_root / "scripts" / "decrypt_eboot.py"
            
            if tool_script.exists():
                if log_fn: log_fn(f"  Decryptage natif via {tool_script.name}...", "info")
                try:
                    subprocess.run([sys.executable, str(tool_script), str(out_eboot), str(eboot_dec_out)], cwd=str(out_dir), check=True, creationflags=subprocess.CREATE_NO_WINDOW)
                    if eboot_dec_out.exists():
                        if log_fn: log_fn("  EBOOT decrypte avec succes en EBOOT_DECRYPTED.BIN.", "ok")
                    else:
                        if log_fn: log_fn("  Erreur: EBOOT_DECRYPTED.BIN introuvable apres decryptage.", "error")
                except Exception as e:
                    if log_fn: log_fn(f"  Erreur lors du decrypt_eboot.py: {e}", "error")
            else:
                if log_fn: log_fn("  Attention: EBOOT crypte, et decrypt_eboot.py introuvable.", "warn")
