"""
P2IS CPK Tool - Outil d'extraction et de reconstruction CPK pour Persona 2 PSP
Basé sur p2is_cpk_tool.py avec corrections de bugs.

Bugs corrigés :
  - max_len = 0 dans compress_crilayla (désactivait totalement la compression LZ)
  - build_cpk compressait TOUJOURS les fichiers même si les originaux ne l'étaient pas
  - Structure du fichier corrigée (imports au bon endroit)
"""
import statistics
import shutil
import struct
    ...
    def __init__(self, data):
        ...
        for i in range(self.column_length):
            ...
            self.columns.append({
                ...
                'storagetype': storagetype, 'fieldtype': fieldtype,
...
class UTFWriter:
    ...
    def add_column(self, name, typeid, constant_data=None):
        ...
        self.columns.append({'name': name, 'typeid': typeid, 'constant_data': constant_data})

    def add_row(self, row_data):
        self.rows.append(row_data)
        for i, col in enumerate(self.columns):
            if (col['typeid'] & COLUMN_STORAGE_MASK) == COLUMN_STORAGE_PERROW and \
               (col['typeid'] & COLUMN_TYPE_MASK) == COLUMN_TYPE_STRING:
                val = row_data[i]
    ...
    def build(self):
        ...
        for col in self.columns:
            ...
            if stype == COLUMN_STORAGE_CONSTANT:
                ...
                row_width += struct.calcsize(COLUMN_TYPE_MAP[ftype])
...
# ─── Compression / Décompression CRILAYLA ─────────────────────────────────────
def compress_crilayla(data: bytes) -> bytes:
    """Compresse des données avec l'algorithme CRILAYLA.
    BUG CORRIGÉ : max_len était 0, ce qui désactivait totalement la recherche de correspondances LZ.
    """
    if len(data) == 0:
        return b""
    ...
    if not source:
        out  = b"CRILAYLA"
        out += struct.pack("<I", 0)  # uncomp_size = 0
        out += struct.pack("<I", 0)  # comp_size = 0
        out += uncomp_header
        return out
    ...
    window_size = 8192
    min_len     = 3
    max_len     = 256  # FIX: était 0, ce qui rendait while mlen < max_len toujours faux!
        ...
        if pos + min_len <= length:
            ...
            if seq in lookup:
                for match_pos in reversed(lookup[seq]):
                    ...
                    while (mlen < max_len
                           and pos + mlen < length
                           and source[pos + mlen] == source[match_pos + mlen]):
                        mlen += 1
...
def decompress_crilayla(data: bytes) -> bytes:
    """Décompresse des données CRILAYLA."""
...
def extract_cpk(cpk_path, out_dir):
    """Extrait tous les fichiers d'une archive CPK.
    Les fichiers compressés CRILAYLA sont automatiquement décompressés.
    """
...
def build_cpk(in_dir, out_cpk_path, original_cpk_path):
    """Reconstruit un CPK en mode 'patch in-place'.
    Seuls les fichiers modifiés (détectés par mtime) sont réinjectés.
    Leur offset est mis à jour dans le TOC.

    BUG CORRIGÉ : les fichiers non compressés à l'origine ne sont PLUS compressés
    avec CRILAYLA (ce qui empêchait le PSP de les lire correctement).
    """
    ...
    with open(out_cpk_path, 'r+b') as f:
        ...
        # Calculer les offsets de chaque colonne dans la ligne
        col_offsets = {}
        curr = 0
        ...
        fo_pattern = '>Q' if (fo_col and fo_col['fieldtype'] == 6) else '>I'

        # Construire une map nom → chemin pour les fichiers locaux
        file_map = {}
        for root, dirs, files in os.walk(in_dir):
            for fname in files:
                file_map[fname] = os.path.join(root, fname)

        # Offset de départ pour les nouvelles données (fin du CPK alignée)
        f.seek(0, 2)
        current_offset = align(f.tell(), 2048)

        # Détection des fichiers modifiés par mtime (médiane = temps d'extraction)
        print("Analyse des fichiers pour patching IN-PLACE...")
        mtimes = []
        for root_dir, _, files in os.walk(in_dir):
            for fname in files:
                if fname.startswith('_') or fname.endswith('.iso') or fname.endswith('.cpk'):
                    continue
        ...
        for row_idx, row in enumerate(toc_utf.rows):
            dirname  = row[cols_toc['DirName']] if 'DirName' in cols_toc else ""
            filename = row[cols_toc['FileName']]
            if not filename:
                continue
            ...
            # Ignorer les fichiers non modifiés (marge de 5 secondes)
            if file_mtime <= median_time + 5:
                continue
            ...
            # ─── BUG CORRIGÉ : ne compresser que si le fichier original l'était ───
            # L'extract_cpk décompresse CRILAYLA → raw_data est toujours décompressé.
            # orig_size < orig_extract  ⟹  original était CRILAYLA compressé
            # orig_size == orig_extract ⟹  original était stocké brut (ne pas compresser)
            was_compressed = (orig_size < orig_extract)

            if was_compressed:
                data_to_write = compress_crilayla(raw_data)
            else:
                data_to_write = raw_data

            new_size    = len(data_to_write)
            new_extract = len(raw_data)

            print(f"  Ecriture de {filename} a l'offset 0x{current_offset:X} "
                  f"({'CRILAYLA' if was_compressed else 'RAW'}, {new_size} octets)")

            # Ecrire les nouvelles données à la fin du CPK
            ...
            current_offset  = align(current_offset + new_size, 2048)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Outil CPK (Pack/Unpack avec CRILAYLA)")
    subparsers = parser.add_subparsers(dest="command", help="Commandes")

    ep = subparsers.add_parser("extract", help="Extraire un fichier CPK")
    ep.add_argument("input_cpk", help="Chemin vers le fichier .cpk")
    ep.add_argument("out_dir",   help="Dossier de sortie")

    rp = subparsers.add_parser("rebuild", help="Reconstruire un fichier CPK")
    rp.add_argument("input_dir",    help="Dossier avec les fichiers modifiés")
    rp.add_argument("original_cpk", help="Chemin vers le .cpk original")
    rp.add_argument("out_cpk",      help="Chemin vers le .cpk de sortie")
